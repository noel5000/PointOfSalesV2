        ...
        public static Tuple<string[],List<List<EntityPropValue>>> GetExcelData<T>(IEnumerable<T> data,string languageId="ES", List<LanguageKey> languageKeys=null) where T : class, new()
        {
            ...
            if (languageKeys != null && languageKeys.Count > 0) 
            {
                headers.ToList().ForEach(x => {
                    string keyVal = languageKeys.FirstOrDefault(k => k.LanguageCode.ToLower() == languageId.ToLower() && k.Key.ToLower() == $"{x.ToLower()}_lbl")?.Value;
                    x = !string.IsNullOrEmpty(keyVal) ? keyVal : x;
                });
            }
            return new Tuple<string[], List<List<EntityPropValue>>>(headers,
                result
                );
        }
    }
