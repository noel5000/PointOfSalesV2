...
@Component({
    ...
    templateUrl: "./schoolIndex.component.html",
    styleUrls: ["../schoolStyles.component.scss"]
})
export class SchoolIndexComponent extends BaseComponent implements OnInit {
    ngOnInit(): void {
        this.verifyUser();
        this.getPagedData(1);
    }
...
this.filterData(e.value,config.id,config.objectType,config.isTranslated);
  ...
  filterIsActive:temp.filterIsActive
    ...
    addNew() {
        this.router.navigateByUrl(`pages/school/add`);
    }
    edit(e:School) {
        this.router.navigateByUrl(`pages/school/edit/${e.id}`);
    }
    source:any={};
    onDeleteConfirm(event:School): void {
